...
describe('Select dropdown viewport constraints', () => {
  ...
  it('moves a visible right-edge dropdown left without shrinking it into a narrow strip', async () => {
  ...
  it('keeps an offscreen-right dropdown usable inside the viewport', async () => {
    setViewportWidth(320)
  ...
  it('uses the full padded viewport when the trigger is wider than the viewport', async () => {
    setViewportWidth(320)
    mockTriggerRect(20, 400)

    const dropdown = await openSelect()

    expect(dropdown).not.toBeNull()
    expect(dropdown?.style.left).toBe('8px')
    expect(dropdown?.style.minWidth).toBe('304px')
