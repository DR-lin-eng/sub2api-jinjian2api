...
import { acquireBodyScrollLock } from '@/core/utils/bodyScrollLock'

const announcementMarkdownStyles = readFileSync(
  resolve(process.cwd(), 'src/core/themes/announcement-markdown.css'),
  'utf8',
)
...
describe('AnnouncementPopup', () => {
  ...
  it('does not clear an overflow lock owned by another modal', () => {
  ...
  it('keeps a later modal lock until its owner releases it', async () => {
    const wrapper = mount(AnnouncementPopup, {
      props: {
        announcement,
        preview: true,
    ...
    const releaseOtherModal = acquireBodyScrollLock()
    await wrapper.setProps({ announcement: null })
    expect(document.body.style.overflow).toBe('hidden')
