export default {
    ops: {
      ...
      settings: {
        ...
        dataRetention: '数据清理与保留',
        userRequestLogs: '用户请求日志',
        userRequestLogsHint: '原始请求与计费明细会在聚合统计完成后清理，不受运维监控清理开关影响。',
        userRequestLogRetentionDays: '用户请求日志保留天数',
        opsMonitoringData: '运维监控数据',
        opsMonitoringDataHint: '包含错误日志、告警事件、系统日志以及分钟和小时指标。',
        enableOpsCleanup: '启用运维监控数据清理',
        ...
        dashboardCards: '仪表盘数据面板',
        displayConcurrency: '展示并发 / 排队',
        displayConcurrencyHint: '关闭后不加载实时并发、排队及分组负载数据。',
        displaySwitchRateTrend: '展示账号切换率趋势',
        displaySwitchRateTrendHint: '关闭后不查询固定 5 小时窗口的账号切换率趋势。',
        displayThroughputTrend: '展示吞吐趋势',
        displayThroughputTrendHint: '关闭后从核心快照中跳过吞吐趋势聚合查询。',
        displayLatencyHistogram: '展示请求时长分布',
        displayLatencyHistogramHint: '关闭后从核心快照中跳过请求时长直方图聚合查询。',
        displayErrorDistribution: '展示错误分布',
        ...
        validation: {
          title: '请先修正以下问题',
          userRequestLogRetentionDaysRange: '用户请求日志保留天数必须在 1-3650 天之间',
      ...
      tooltips: {
        ...
        goroutines:
          'Go 运行时的协程数量（轻量级线程）。高并发和长连接会自然增加协程，单看数量不等于系统负担过重。当前展示阈值：<30000 正常；30000-49999 警告；>=50000 严重。判断异常时应同时观察队列、延迟和持续增长趋势。',
        cpu: 'CPU 使用率，显示系统处理器的负载情况。',
        memory: '内存使用率，包括已使用和总可用内存。',
        db: '数据库连接池状态，包括活跃连接、空闲连接和等待连接数。',
        redis: 'Redis 连接池状态，百分比按活跃连接数计算，下方显示总连接和空闲连接数。',
