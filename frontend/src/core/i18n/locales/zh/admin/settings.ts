export default {
    settings: {
      ...
      features: {
        channelMonitor: {
          title: '渠道监控',
          description: '定期对配置的渠道发起健康检查，并在管理员控制台展示可用性与延迟。',
      ...
      security: {
        ...
        passkey: 'Passkey 登录',
        passkeyHint: '当依赖方配置有效时，允许本地管理员无密码登录并管理 Passkey。',
        ...
        stepUpHint: '开启后，账号/代理导出、备份创建与下载、S3 配置修改等敏感操作需要先完成 TOTP 二次验证（15 分钟内有效）。开启前需当前管理员已启用 2FA；关闭该开关本身也需要二次验证。',
      ...
      panelRateLimit: {
        ...
        proxySafeNote: '登录后的接口按本地管理员计数，与来源 IP 无关；公开接口按真实客户端 IP 计数，回环与内网反代地址会自动跳过。',
        enabled: '启用面板接口限流',
        enabledHint: '对本地管理员的面板接口限流；超出阈值返回 429，窗口重置后自动恢复。',
        userRpm: '管理员请求上限',
        userRpmHint: '本地管理员每分钟允许的面板 API 请求总数，正常页面操作远达不到该阈值；0 表示不限制。',
        heavyRpm: '管理员重查询上限',
        heavyRpmHint: '本地管理员每分钟允许的用量/仪表盘等聚合统计查询次数（这类请求对数据库压力最大）；0 表示不限制。',
        publicIpRpm: '公开接口每 IP 上限',
        publicIpRpmHint: '无需登录的公开接口（如站点公开设置）每个真实客户端 IP 每分钟的请求上限；0 表示不限制。',
        perMinute: '次/分钟',
        exemptAdmin: '管理员豁免',
        exemptAdminHint: '开启后本地管理员不受面板限流约束，避免批量运维操作被误拦。',
      ...
      site: {
        ...
        tablePreferencesDescription: '设置管理端共享表格组件的默认分页行为',
        ...
        customEndpoints: {
          title: '自定义端点',
          description: '添加可在「API Keys」页面快速复制的 API 端点地址',
          name: '名称',
