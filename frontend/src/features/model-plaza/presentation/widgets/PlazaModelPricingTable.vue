    ...
    <table class="w-full min-w-[860px] table-fixed border-collapse text-sm tabular-nums">
      ...
      <tbody>
        ...
        >
            ...
            <td class="pz-cell px-3 py-2.5 align-middle font-mono font-semibold text-gray-900 dark:text-gray-50">
              <template v-if="tokenIntervals(m).length">
                ...
                >
                  ...
                  {{ paidPerMillion(iv.inputPrice) }}
                </div>
              </template>
              <template v-else>{{ paidPerMillion(m.pricing?.inputPrice) }}</template>
            </td>
            <td class="pz-cell px-3 py-2.5 align-middle font-mono font-semibold text-gray-900 dark:text-gray-50">
              <template v-if="tokenIntervals(m).length">
                <div
                ...
                >
                  <span class="mr-1 font-sans font-normal text-gray-400 dark:text-dark-500">{{ tierLabel(iv) }}</span>
                  {{ paidPerMillion(iv.outputPrice) }}
                </div>
              </template>
              <template v-else>{{ paidPerMillion(m.pricing?.outputPrice) }}</template>
            </td>
          ...
          >
            {{ official(m.officialPricing?.inputPrice) }}
          </td>
          <td class="px-3 py-2.5 align-middle font-mono text-xs text-gray-500 dark:text-dark-400">
            {{ official(m.officialPricing?.outputPrice) }}
          </td>
          <td class="px-3 py-2.5 align-middle">
            <div
              v-if="m.officialPricing && hasOfficialCache(m.officialPricing)"
              class="space-y-0.5 font-mono text-xs text-gray-500 dark:text-dark-400"
            >
              <div>
                <span class="mr-1 font-sans font-normal text-gray-400 dark:text-dark-500">{{ t('modelPlaza.table.cacheWrite') }}</span>
                {{ official(m.officialPricing.cacheWritePrice)
                }}<template v-if="m.officialPricing.cacheWrite1hPrice != null"
                  ><span class="font-sans text-gray-400 dark:text-dark-500"> (1h </span>{{ official(m.officialPricing.cacheWrite1hPrice)
                  }}<span class="font-sans text-gray-400 dark:text-dark-500">)</span></template
                >
              </div>
              <div>
                <span class="mr-1 font-sans font-normal text-gray-400 dark:text-dark-500">{{ t('modelPlaza.table.cacheRead') }}</span>
                {{ official(m.officialPricing.cacheReadPrice) }}
              </div>
            </div>
            <span v-else class="text-gray-400 dark:text-dark-500">-</span>
          </td>
...
import { platformAccentColor } from '@/core/utils/platformColors'
import {
  BILLING_MODE_TOKEN,
  BILLING_MODE_IMAGE,
  type BillingMode
} from '@/core/constants/channel'
import type { ModelPlazaModel } from '@/features/model-plaza/domain/models/modelPlazaModel'
import type { ModelPlazaPricingInterval } from '@/features/model-plaza/domain/models/modelPlazaPricingInterval'
...
const sortedModels = computed(() => {
  return [...props.models].sort((a, b) => {
    const pa = a.officialPricing?.outputPrice ?? null
    const pb = b.officialPricing?.outputPrice ?? null
...
function billingMode(m: ModelPlazaModel): BillingMode {
  return (m.pricing?.billingMode || BILLING_MODE_TOKEN) as BillingMode
}

function billingModeLabel(m: ModelPlazaModel): string {
...
function perUnitSuffix(m: ModelPlazaModel): string {
  return billingMode(m) === BILLING_MODE_IMAGE
    ? t('modelPlaza.table.perUnitImage')
    : t('modelPlaza.table.perUnitRequest')
}

function hasCachePricing(m: ModelPlazaModel): boolean {
  return m.pricing?.cacheWritePrice != null || m.pricing?.cacheReadPrice != null
}

function hasOfficialCache(o: NonNullable<ModelPlazaModel['officialPricing']>): boolean {
  return o.cacheWritePrice != null || o.cacheReadPrice != null || o.cacheWrite1hPrice != null
}

/** token 模式的阶梯定价(内联进输入/输出列)。 */
function tokenIntervals(m: ModelPlazaModel): ModelPlazaPricingInterval[] {
  return m.pricing?.intervals ?? []
}

/** 按次/按图模式的阶梯定价(仅保留配了按次价的档位)。 */
function requestIntervals(m: ModelPlazaModel): ModelPlazaPricingInterval[] {
  return (m.pricing?.intervals ?? []).filter((iv) => iv.perRequestPrice != null)
}

/** 档位标签:优先管理员配置的 tierLabel,否则按 token 区间生成(≤200K / >200K / 200K–1M)。 */
function tierLabel(iv: ModelPlazaPricingInterval): string {
  if (iv.tierLabel) return iv.tierLabel
  const { minTokens: min, maxTokens: max } = iv
