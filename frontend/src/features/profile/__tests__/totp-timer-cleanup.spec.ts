...
import TotpSetupDialog from '@/features/profile/presentation/widgets/TotpSetupDialog.vue'
...
describe('TOTP 弹窗定时器清理', () => {
  ...
  it('TotpSetupDialog 卸载时清理倒计时定时器', async () => {
    const wrapper = mount(TotpSetupDialog)
    await flushPromises()
  ...
  it('TotpSetupDialog 失败时改用 toast 并不渲染内联错误', async () => {
