          ...
          >
            <div class="min-w-0">
              ...
              <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                {{ t('profile.passkey.createdAt', { date: formatDate(credential.createdAt) }) }}
                <template v-if="credential.lastUsedAt">
                  · {{ t('profile.passkey.lastUsed', { date: formatDate(credential.lastUsedAt) }) }}
                </template>
              </p>
            </div>
...
import { passkeysActionRepository } from '@/features/passkeys/data/repositories/passkeysActionRepositoryImpl'
import { passkeysQueryRepository } from '@/features/passkeys/data/repositories/passkeysQueryRepositoryImpl'
...
const supported = passkeysQueryRepository.isSupported()
...
async function loadCredentials(): Promise<void> {
  ...
  loading.value = true
  try {
    credentials.value = await passkeysQueryRepository.list()
  } catch (error) {
...
async function addPasskey(): Promise<void> {
  ...
  busy.value = true
  try {
    await passkeysActionRepository.register({
...
async function renamePasskey(credential: PasskeyCredentialSummary): Promise<void> {
  ...
  busy.value = true
  try {
    await passkeysActionRepository.rename(credential.id, { name })
...
async function confirmDelete(): Promise<void> {
  ...
  busy.value = true
  try {
    await passkeysActionRepository.remove(credential.id, { password: deletePassword.value })
