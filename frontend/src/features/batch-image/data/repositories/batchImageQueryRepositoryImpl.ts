...
class BatchImageQueryRepositoryImpl implements BatchImageQueryRepository {
  ...
  getById = async (apiKey: string, batchId: string) : Promise<BatchImageJob>  => {
    return (await this.ds.getById(apiKey, batchId)).toEntity()
  }

  list = async (apiKey: string, options?: ListBatchImageJobsRequest) : Promise<BatchImageJobsPage>  => {
    const result = await this.ds.list(apiKey, options)
    return { data: result.data.map(dto => dto.toEntity()), has_more: result.has_more }
  }

  listModels = async (apiKey: string) : Promise<BatchImageModel[]>  => {
    const result = await this.ds.listModels(apiKey)
    return result.data.map(dto => dto.toEntity())
  }

  listItems = async (apiKey: string, batchId: string, status?: string) : Promise<BatchImageItemsPage>  => {
    const result = await this.ds.listItems(apiKey, batchId, status)
    return { data: result.data.map(dto => dto.toEntity()), has_more: result.has_more }
  }

  getItemContent = async (apiKey: string, batchId: string, customId: string, imageIndex?: number) : Promise<Blob>  => {
