...
vi.mock('@/features/admin-accounts/presentation/stores/adminAccountsQueryStore', () => ({
  useAdminAccountsQueryStore: () => ({
    getAntigravityDefaultModelMapping,
  }),
}))
...
function mountModal(extraProps: Record<string, unknown> = {}) {
  return mount(BulkEditAccountDialog, {
    props: {
      show: true,
      accountIds: [1, 2],
...
describe('BulkEditAccountDialog', () => {
  beforeEach(() => {
    setActivePinia(createPinia())
    bulkUpdate.mockReset()
    checkMixedChannelRisk.mockReset()
    getAntigravityDefaultModelMapping.mockReset()
    getAntigravityDefaultModelMapping.mockResolvedValue({})
