...
import AccountsPage from '@/features/admin-accounts/presentation/pages/AccountsPage.vue'
...
// 外审 F2:AccountActionMenu emit 'create-spark-shadow',但 AccountsPage 此前未监听,
...
describe('admin AccountsPage — 外审 F2:spark 影子创建接线', () => {
  beforeEach(() => {
    localStorage.clear()
...
describe('admin AccountsPage — 账号行展示', () => {
  beforeEach(() => {
    localStorage.clear()
    for (const fn of [listAccounts, listWithEtag, getBatchTodayStats, getAllProxies, getAllGroups, duplicateAccount, createSparkShadow, showSuccess, showError]) {
      fn.mockReset()
