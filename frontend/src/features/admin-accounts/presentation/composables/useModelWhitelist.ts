...
export async function fetchAntigravityDefaultMappings(): Promise<{ from: string; to: string }[]> {
  ...
  try {
    const store = useAdminAccountsQueryStore()
    const mapping = await store.getAntigravityDefaultModelMapping()
    _antigravityDefaultMappingsCache = Object.entries(mapping).map(([from, to]) => ({ from, to: String(to) }))
  } catch (e) {
