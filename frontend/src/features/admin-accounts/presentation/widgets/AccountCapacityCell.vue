<template>
  <div class="flex flex-col gap-0.5">
    ...
    <!-- 5h窗口费用限制 -->
    <CapacityBadge v-if="showWindowCost" :color-class="windowCostClass" :tooltip="windowCostTooltip" :current="'$' + formatCost(currentWindowCost)" :max="'$' + formatCost(account.windowCostLimit)">
      <svg class="h-2.5 w-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <!-- 会话数量限制 -->
    <CapacityBadge v-if="showSessionLimit" :color-class="sessionLimitClass" :tooltip="sessionLimitTooltip" :current="activeSessions" :max="account.maxSessions!">
      <svg class="h-2.5 w-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <!-- RPM 限制 -->
    <CapacityBadge v-if="showRpmLimit" :color-class="rpmClass" :tooltip="rpmTooltip" :current="currentRPM" :max="account.baseRpm!" :suffix="rpmStrategyTag">
    ...
    <!-- API Key 账号配额限制 -->
    <QuotaBadge v-if="showDailyQuota" :used="account.quotaDailyUsed ?? 0" :limit="account.quotaDailyLimit!" label="D" />
    <QuotaBadge v-if="showWeeklyQuota" :used="account.quotaWeeklyUsed ?? 0" :limit="account.quotaWeeklyLimit!" label="W" />
    <QuotaBadge v-if="showTotalQuota" :used="account.quotaUsed ?? 0" :limit="account.quotaLimit!" />
  </div>
</template>
...
// ====== 并发 ======
const currentConcurrency = computed(() => props.account.currentConcurrency || 0)
...
const showWindowCost = computed(() =>
  ...
  props.account.windowCostLimit != null &&
  props.account.windowCostLimit > 0
)

const currentWindowCost = computed(() => props.account.currentWindowCost ?? 0)

const windowCostClass = computed(() => {
  if (!showWindowCost.value) return ''
  const current = currentWindowCost.value
  const limit = props.account.windowCostLimit || 0
  const reserve = props.account.windowCostStickyReserve || 10
...
const showSessionLimit = computed(() =>
  isAnthropicOAuthOrSetupToken.value &&
  props.account.maxSessions != null &&
  props.account.maxSessions > 0
)

const activeSessions = computed(() => props.account.activeSessions ?? 0)

const sessionLimitClass = computed(() => {
  if (!showSessionLimit.value) return ''
  const current = activeSessions.value
  const max = props.account.maxSessions || 0
  if (current >= max) return 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'
...
const sessionLimitTooltip = computed(() => {
  ...
  const current = activeSessions.value
  const max = props.account.maxSessions || 0
  const idle = props.account.sessionIdleTimeoutMinutes || 5
...
const showRpmLimit = computed(() =>
  isAnthropicOAuthOrSetupToken.value &&
  props.account.baseRpm != null &&
  props.account.baseRpm > 0
)

const currentRPM = computed(() => props.account.currentRpm ?? 0)
const rpmStrategy = computed(() => props.account.rpmStrategy || 'tiered')
const rpmStrategyTag = computed(() => rpmStrategy.value === 'sticky_exempt' ? '[S]' : '[T]')

const rpmBuffer = computed(() => {
  const base = props.account.baseRpm || 0
  return props.account.rpmStickyBuffer ?? (base > 0 ? Math.max(1, Math.floor(base / 5)) : 0)
})
...
const showDailyQuota = computed(() =>
  isQuotaEligible.value && props.account.quotaDailyLimit != null && props.account.quotaDailyLimit > 0
)
const showWeeklyQuota = computed(() =>
  isQuotaEligible.value && props.account.quotaWeeklyLimit != null && props.account.quotaWeeklyLimit > 0
)
const showTotalQuota = computed(() =>
  isQuotaEligible.value && props.account.quotaLimit != null && props.account.quotaLimit > 0
)
</script>
