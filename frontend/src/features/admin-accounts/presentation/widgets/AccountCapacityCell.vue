<template>
  <div class="flex flex-col gap-0.5">
    <!-- 并发槽位 -->
    <CapacityBadge :color-class="concurrencyClass" :current="currentConcurrency" :max="effectiveConcurrency" :tooltip="cpaCapacityTooltip">
...
const cpaCapacityTooltip = computed(() => {
  if (!cpaCapacity.value) return undefined
  const key = cpaCapacity.value.state === 'fresh'
    ? 'admin.accounts.capacity.cpaFresh'
    : cpaCapacity.value.state === 'stale'
      ? 'admin.accounts.capacity.cpaStale'
      : 'admin.accounts.capacity.cpaUnavailable'
  return t(key, { fallback: props.account.concurrency })
})
