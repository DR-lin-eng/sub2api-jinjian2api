...
import {
  create as createScheduledTestPlan,
  deletePlan,
  listByAccount,
  listResults,
  update as updateScheduledTestPlan
...
const loadPlans = async () => {
  ...
  loading.value = true
  try {
    plans.value = await listByAccount(props.accountId)
  } catch (error: any) {
...
const handleToggleEnabled = async (plan: ScheduledTestPlan, enabled: boolean) => {
  try {
    const updated = await updateScheduledTestPlan(plan.id, { enabled })
...
const handleEdit = async () => {
  ...
  updating.value = true
  try {
    const updated = await updateScheduledTestPlan(editingPlanId.value, {
...
const toggleExpand = async (planId: number) => {
  ...
  try {
    results.value = await listResults(planId, 20)
  } catch (error: any) {
