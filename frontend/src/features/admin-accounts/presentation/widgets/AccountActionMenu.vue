      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button v-if="hasCPAMode" @click="$emit('sync-cpa', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="sync" size="sm" class="text-cyan-600" />
              {{ t('admin.accounts.syncCPA') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'sync-cpa', 'stats', 'schedule', 'duplicate', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'create-spark-shadow'])
const { t } = useI18n()
const canDuplicate = computed(() => {
  if (!props.account || props.account.parent_account_id != null) return false
  return ['apikey', 'upstream', 'bedrock', 'service_account'].includes(props.account.type)
})
const hasCPAMode = computed(() => props.account?.type === 'apikey' && props.account.credentials?.cpa_mode === true)
