<template>
  <div
    v-if="stats && stats.totalRequests > 0"
  ...
  >
    <div class="flex items-center justify-between gap-2">
      ...
      <span class="font-medium text-gray-800 dark:text-gray-200">{{ formatTTFT(stats.avgFirstTokenMs) }}</span>
    </div>
    <div class="flex items-center justify-between gap-2">
      <span class="text-gray-500 dark:text-gray-400">{{ t('admin.accounts.hourlyUsage.successRate') }}</span>
      <span :class="successRateClass">{{ formatSuccessRate(stats.successRate) }}</span>
    </div>
    <div class="flex items-center justify-between gap-2">
      <span class="text-gray-500 dark:text-gray-400">{{ t('admin.accounts.hourlyUsage.error4xx') }}</span>
      <span class="font-medium text-amber-600 dark:text-amber-400">{{ stats.error4xx }}</span>
    </div>
    <div class="flex items-center justify-between gap-2">
      <span class="text-gray-500 dark:text-gray-400">{{ t('admin.accounts.hourlyUsage.error5xx') }}</span>
      <span class="font-medium text-red-600 dark:text-red-400">{{ stats.error5xx }}</span>
    </div>
  </div>
...
import type { AccountHourlyUsageStats } from '@/features/admin-accounts/domain/models/accountHourlyUsageStats'
