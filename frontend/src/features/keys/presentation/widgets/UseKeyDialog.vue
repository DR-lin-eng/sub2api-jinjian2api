...
const createOpenCodeFile = (
  platform: OpenCodeProviderId,
  baseUrl: string,
  apiKey: string,
  pathLabel?: string
): FileConfig => generateOpenCodeConfig({
  platform,
  baseUrl,
  apiKey,
  pathLabel,
...
const currentFiles = computed((): FileConfig[] => {
  ...
  if (activeClientTab.value === 'opencode') {
    switch (props.platform) {
      case 'anthropic':
        return [createOpenCodeFile('anthropic', apiBase, apiKey)]
      case 'openai':
        return [createOpenCodeFile('openai', apiBase, apiKey)]
      case 'gemini':
        return [createOpenCodeFile('gemini', geminiBase, apiKey)]
      case 'antigravity':
        return [
          createOpenCodeFile('antigravity-claude', antigravityBase, apiKey, 'opencode.json (Claude)'),
          createOpenCodeFile('antigravity-gemini', antigravityGeminiBase, apiKey, 'opencode.json (Gemini)')
        ]
      case 'grok':
        return [createOpenCodeFile('grok', apiBase, apiKey)]
      default:
        return [createOpenCodeFile('openai', apiBase, apiKey)]
    }
  }
