...
import AvailableChannelsTable from '../presentation/widgets/AvailableChannelsTable.vue'
import type { UserAvailableChannel } from '@/features/channels-user/data/datasources/channelsUserDatasource'
...
const componentPath = resolve(
  ...
  '../presentation/widgets/AvailableChannelsTable.vue',
)
...
function mountTable(props = {}, desktop = true) {
...
describe('AvailableChannelsTable responsive surfaces', () => {
  it('keeps the five-column table as the desktop-only surface', () => {
    ...
    expect(wrapper.find('[data-testid="mobile-channels"]').exists()).toBe(false)
  ...
  it('provides loading and empty states on the mounted responsive surface', async () => {
    const desktop = mountTable({ loading: true, rows: [] }, true)
    expect(desktop.get('[data-testid="desktop-channels"] [data-icon="refresh"]')).toBeTruthy()
    await desktop.setProps({ loading: false })
    expect(desktop.get('[data-testid="desktop-channels"]').text()).toContain('No channels')

    const mobile = mountTable({ loading: true, rows: [] }, false)
    expect(mobile.get('[data-testid="mobile-loading"] [data-icon="refresh"]')).toBeTruthy()
    await mobile.setProps({ loading: false })
    expect(mobile.get('[data-testid="mobile-empty"]').text()).toContain('No channels')
  })
})
