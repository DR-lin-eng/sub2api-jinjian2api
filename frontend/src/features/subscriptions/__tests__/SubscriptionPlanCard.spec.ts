...
describe('SubscriptionPlanCard', () => {
  ...
  it('renders plural admin-form validity units instead of mislabeled days (#4607)', () => {
    expect(mountPlanCard('openai', { validityDays: 1, validityUnit: 'months' }).text()).toContain('/ payment.perMonth')
    expect(mountPlanCard('openai', { validityDays: 3, validityUnit: 'months' }).text()).toContain('/ 3payment.months')
    expect(mountPlanCard('openai', { validityDays: 2, validityUnit: 'weeks' }).text()).toContain('/ 2payment.weeks')
    expect(mountPlanCard('openai', { validityDays: 30, validityUnit: 'day' }).text()).toContain('/ 30payment.days')
  })

  it('uses the configured currency symbol while preserving USD for legacy plans', () => {
    const cnyPlan = mountPlanCard('openai', { currency: 'CNY', originalPrice: 20 }).text()
