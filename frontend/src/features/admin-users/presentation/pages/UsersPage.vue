...
import UserCreateModal from '@/features/admin-users/presentation/widgets/UserCreateDialog.vue'
import UserEditModal from '@/features/admin-users/presentation/widgets/UserEditDialog.vue'
import BulkEditUserModal from '@/features/admin-users/presentation/widgets/BulkEditUserDialog.vue'
import UserPlatformQuotaModal from '@/features/admin-users/presentation/widgets/UserPlatformQuotaDialog.vue'
import UserApiKeysModal from '@/features/admin-users/presentation/widgets/UserApiKeysDialog.vue'
import UserAllowedGroupsModal from '@/features/admin-users/presentation/widgets/UserAllowedGroupsDialog.vue'
...
const refreshCurrentPageSecondaryData = () => {
  const userIds = users.value.map((u: any) => u.id)
  if (userIds.length === 0) return
...
const loadUsers = async () => {
  ...
  try {
    ...
    if (response.items.length > 0) {
      const userIds = response.items.map((u: any) => u.id)
      const seq = ++secondaryDataSeq
      window.setTimeout(() => {
