...
import { LoginView } from '@/features/auth/presentation/pages'
</script>
```
...
<script setup lang="ts">
import { RegisterView } from '@/features/auth/presentation/pages'
</script>
```
...
- `AuthLayout` - Layout wrapper for auth pages from `@/common/widgets/layout`

### Stores

- `authStore` - Authentication state management from `@/features/auth/presentation/stores/authStore`
- `appStore` - Application state and toasts from `@/core/stores/appStore`
...
- [Auth Store Documentation](/src/core/stores/README.md#auth-store)
- [AuthLayout Component](/src/common/widgets/layout/README.md#authlayout)
- [Router Configuration](/src/core/routes/index.ts)
