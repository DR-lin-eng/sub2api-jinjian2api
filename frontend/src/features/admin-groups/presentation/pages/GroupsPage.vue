...
const createModelsListSelectedCount = computed(
  () => createModelsListState.items.filter((item: { selected: boolean }) => item.selected).length,
);
const editModelsListSelectedCount = computed(
  () => editModelsListState.items.filter((item: { selected: boolean }) => item.selected).length,
);
