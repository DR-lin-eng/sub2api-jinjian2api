...
共享编辑器的 `BaseDialog`、`form` 和 footer 由 `GroupEditorDialog.vue` 持有；核心路由、Antigravity、Anthropic/OpenAI、账号过滤与模型路由字段分别由同目录静态 widget 展示。子 widget 只消费既有 `groupEditorContext`，不得新增请求、watcher 或第二份表单状态。
