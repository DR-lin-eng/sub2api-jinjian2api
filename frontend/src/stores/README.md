...
| 文件 | Store ID | 职责 |
| --- | --- | --- |
| `auth.ts` | `auth` | 当前用户、内存 access token、会话恢复和用户权限投影 |
| `app.ts` | `app` | 公开站点设置、主题/壳层状态、全局 toast 和运行模式 |
| `adminSettings.ts` | `adminSettings` | 管理端设置与自定义菜单缓存 |
| `adminCompliance.ts` | `adminCompliance` | 管理员合规确认状态与阻断流程 |
| `subscriptions.ts` | `subscriptions` | 活跃订阅缓存、刷新和轮询生命周期 |
| `payment.ts` | `payment` | 支付配置、套餐和当前订单 |
| `announcements.ts` | `announcements` | 公告加载和展示状态 |
| `onboarding.ts` | `onboarding` | 新手引导实例和步骤控制 |
| `index.ts` | - | Store 与公共类型的统一导出 |
...
Store 可以调用 `src/api/`，但不应绕过共享 client。Store 不直接操作 DOM；复杂 UI 行为使用 composable。
...
4. 从 `index.ts` 导出公共 store。
5. 添加测试，覆盖首次加载、并发加载、失败、刷新、清理和用户切换。

认证或 API 统一行为应优先修改 `src/api/client.ts`、`tokenStore.ts` 或 `sessionRefresh.ts`，不要为其新建平行 store。
