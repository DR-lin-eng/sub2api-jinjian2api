...
run_api_curl() {
    CURL_ARGS_LOG="$1" HOME="$TEMP_DIR/home" PATH="$TEMP_DIR:$PATH" UPDATE_GITHUB_TOKEN="${2:-}" \
        ...
        "$ROOT_DIR/deploy/install.sh" "https://api.github.com/repos/DR-lin-eng/sub2api-jinjian2api/releases/latest"
}
...
test "$(grep -Fxc 'https://api.github.com/repos/DR-lin-eng/sub2api-jinjian2api/releases/latest' "$TEMP_DIR/authenticated")" -eq 1
...
test "$(grep -Fxc 'https://api.github.com/repos/DR-lin-eng/sub2api-jinjian2api/releases/latest' "$TEMP_DIR/anonymous")" -eq 1
...
assert_unsafe_invocation_rejected non-api -s \
    "https://github.com/DR-lin-eng/sub2api-jinjian2api/releases/download/v1/asset"
assert_unsafe_invocation_rejected mixed-host -s \
    "https://api.github.com/repos/DR-lin-eng/sub2api-jinjian2api/releases/latest" \
    "https://example.com/collect"
assert_unsafe_invocation_rejected multiple-api -s \
    "https://api.github.com/repos/DR-lin-eng/sub2api-jinjian2api/releases/latest" \
    "https://api.github.com/repos/DR-lin-eng/sub2api-jinjian2api/releases"
...
# Release assets stay anonymous and fail closed on missing signed metadata.
...
grep -Fq 'curl -fsSL --proto '\''=https'\'' --tlsv1.2 "$signature_url"' "$ROOT_DIR/deploy/install.sh"
grep -Fq 'openssl pkeyutl -verify' "$ROOT_DIR/deploy/install.sh"
