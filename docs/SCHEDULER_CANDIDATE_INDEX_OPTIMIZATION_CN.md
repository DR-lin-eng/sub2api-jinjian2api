...
> 当前网关运行策略（2026-08）：账号池面向高并发固定使用 V2 候选索引、有限优先准入、会话缓存亲和和健康感知选号。历史设置 API 字段仍保留用于滚动升级兼容，但不再作为运行时开关；服务启动时会把旧的 legacy 状态提升为 V2。OpenAI/Codex 的 429/502 健康隔离由请求级 failover 和短时模型熔断负责。
...
- Redis 仍保存 `scheduler_v2_enabled` 兼容字段和状态机，运行时固定保持 V2；管理端不再提供切换控件。
- 固定单请求最多收集 64 个合格候选、检查 256 个原始账号；预算由代码统一维护，管理员不能在运行时改写。
- V2 是全平台、全账号类型、全实例生效，不是 OpenAI 专用开关。
...
- 旧 legacy 状态被提升为 V2 前会清除可能过期的旧快照就绪标记，避免滚动升级读到陈旧数据。
...
旧 `legacy/disabled` 只作为滚动升级和 Redis 丢失时的兼容状态；服务启动会把它提升为 `v2/building`，构建成功后进入 `v2/active`。管理员不能切回 legacy。

关键点：进入 `v2/building` 的瞬间，后续请求已经只走 V2。系统不会先在旧版旁边“预热一套”再切换，因此不存在影子阶段。

### 5.2 启动/恢复流程

1. 服务启动或检测到旧状态时，将 Redis 全局状态发布为 `v2/building`。
...
旧 API 仍可读写 `scheduler_v2_enabled` 和预算字段，以便滚动升级；写入不会改变运行时固定策略，也不会重新启用旧快照路径。

### 5.4 启动、多实例与 Redis 状态丢失

- 启动时数据库设置是持久化目标，Redis 是运行时全局权威状态。
- 数据库目标为 V2 且 Redis 已是 `v2/active` 时，只复用索引版本正确且完整的 bucket。
- Redis 状态键缺失时，服务发布固定的 V2 building 状态，并触发受全局锁保护的索引恢复。
...
- `scheduler_v2_candidate_limit=64`：最终交给平台动态策略的合格候选窗口；
- `scheduler_v2_scan_limit=256`：模型路由 ID、首批索引账号和所有扩展分页合计最多检查的原始账号数。

旧客户端提交的预算字段会被归一化到 64/256；预算不在请求热路径读取数据库。
...
- 进程启动校验、Redis 状态恢复、索引版本变化或 bucket 损坏会定向或全量重建。
...
旧设置写入失败只影响兼容设置文档，不改变已运行的 V2 候选索引状态。
...
go test ./...
go test -tags=unit ./...
go test ./internal/application/service -run Scheduler -count=1
go test ./internal/infrastructure/repository -run SchedulerCacheV2 -count=1
SCHEDULER_FIXTURE_DIR=/home/ubuntu-24/sub2api-codex/.cache/scheduler-fixture \
  go test -tags=schedulerfixture ./internal/application/service \
  -run SchedulerProductionFixture -count=1 -v
go test ./internal/infrastructure/repository -run '^$' \
...
- V2 运行期间不维护旧快照；系统不会关闭 V2 或回退到旧快照。
