...
func TestLoadOpenAIFirstOutputTimeoutsCanBeDisabledFromEnv(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("GATEWAY_OPENAI_FIRST_OUTPUT_TIMEOUT_SECONDS", "0")
	t.Setenv("GATEWAY_OPENAI_HIGH_EFFORT_FIRST_OUTPUT_TIMEOUT_SECONDS", "0")
...
func TestLoadDefaultOpenAIHTTP2Enabled(t *testing.T) {
	...
	require.False(t, cfg.Gateway.OpenAIProxyStreamCircuit.Enabled)
...
func TestLoadOpenAIProxyStreamCircuitFromEnv(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("GATEWAY_OPENAI_PROXY_STREAM_CIRCUIT_ENABLED", "true")
	...
	cfg, err := Load()
	require.NoError(t, err)
	require.True(t, cfg.Gateway.OpenAIProxyStreamCircuit.Enabled)
...
func TestValidateConfigErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "gateway openai proxy stream circuit enabled zero value",
			mutate: func(c *Config) {
				c.Gateway.OpenAIProxyStreamCircuit.Enabled = true
				c.Gateway.OpenAIProxyStreamCircuit.FailureThreshold = 0
			},
			wantErr: "gateway.openai_proxy_stream_circuit values must be positive when enabled",
		},
		{
