...
const (
	...
	ollamaCloudUsageDisabledSettingsTTL    = 5 * time.Minute
	ollamaCloudUsageEnabledSettingsTTL     = time.Minute
...
type OllamaCloudUsageSnapshot struct {
	...
	NextRefreshUnix int64                 `json:"next_refresh_unix,omitempty"`
...
type OllamaCloudUsageService struct {
	...
	settingsCached         bool
	settingsCache          OllamaCloudUsageSettings
	settingsCacheExpiresAt time.Time
	settingsRefreshGroup   singleflight.Group
	now                    func() time.Time
...
func (s *OllamaCloudUsageService) GetSettings(ctx context.Context) (*OllamaCloudUsageSettings, error) {
	...
	settings, err := s.settingService.GetOllamaCloudUsageSettings(ctx)
	if err != nil {
		return nil, err
	}
	s.storeRunnerSettings(settings)
	return cloneOllamaCloudUsageSettings(settings), nil
}
...
func (s *OllamaCloudUsageService) UpdateSettings(ctx context.Context, settings *OllamaCloudUsageSettings) error {
	...
	if err := s.settingService.SetOllamaCloudUsageSettings(ctx, settings); err != nil {
		return err
	}
	s.storeRunnerSettings(settings)
	return nil
...
func (s *OllamaCloudUsageService) getRunnerSettings(ctx context.Context) (*OllamaCloudUsageSettings, error) {
	if s == nil || s.settingService == nil {
		return defaultOllamaCloudUsageSettings(), nil
	}
	if settings, ok := s.cachedRunnerSettings(); ok {
		return settings, nil
	}
	value, err, _ := s.settingsRefreshGroup.Do("settings", func() (any, error) {
		if settings, ok := s.cachedRunnerSettings(); ok {
			return settings, nil
		}
		settings, loadErr := s.settingService.GetOllamaCloudUsageSettings(ctx)
		if loadErr != nil {
			return nil, loadErr
		}
		s.storeRunnerSettings(settings)
	...
	settings, ok := value.(*OllamaCloudUsageSettings)
	if !ok || settings == nil {
		return nil, fmt.Errorf("invalid Ollama Cloud usage settings cache result")
	}
	return cloneOllamaCloudUsageSettings(settings), nil
}

func (s *OllamaCloudUsageService) cachedRunnerSettings() (*OllamaCloudUsageSettings, bool) {
	if s == nil {
		return nil, false
	}
	now := s.currentTime()
	...
	if !s.settingsCached || !now.Before(s.settingsCacheExpiresAt) {
		return nil, false
	}
...
func (s *OllamaCloudUsageService) storeRunnerSettings(settings *OllamaCloudUsageSettings) {
	if s == nil || settings == nil {
		return
	}
	copy := *settings
	ttl := ollamaCloudUsageDisabledSettingsTTL
	if copy.Enabled {
		ttl = ollamaCloudUsageEnabledSettingsTTL
	}
	expiresAt := s.currentTime().Add(ttl)
	s.settingsMu.Lock()
	s.settingsCache = copy
	s.settingsCached = true
	s.settingsCacheExpiresAt = expiresAt
	s.settingsMu.Unlock()
}

func cloneOllamaCloudUsageSettings(settings *OllamaCloudUsageSettings) *OllamaCloudUsageSettings {
	if settings == nil {
		return nil
	}
	copy := *settings
...
func (s *OllamaCloudUsageService) RunDue(ctx context.Context) error {
	...
	settings, err := s.getRunnerSettings(ctx)
	if err != nil {
		return err
	}
	if !settings.Enabled {
...
func (s *OllamaCloudUsageService) updateSnapshot(ctx context.Context, account *Account, snapshot *OllamaCloudUsageSnapshot) error {
	...
	if snapshot != nil {
		if snapshot.NextRefreshAt.IsZero() {
			snapshot.NextRefreshUnix = 0
		} else {
			snapshot.NextRefreshUnix = snapshot.NextRefreshAt.Unix()
		}
	}
