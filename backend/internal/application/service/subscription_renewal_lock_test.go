...
func TestAssignSubscriptionDoesNotExtendRowRenewedAfterStaleRead(t *testing.T) {
	now := time.Date(2026, 8, 2, 12, 0, 0, 0, time.UTC)
	current := UserSubscription{
		ID: 37, UserID: 41, GroupID: 43, StartsAt: now.AddDate(0, 0, -10), ExpiresAt: now.AddDate(0, 0, 20),
		Status: SubscriptionStatusActive, Notes: "already renewed",
	}
	repo := &lockingRenewalRepo{
	...
	sub, reused, err := svc.assignSubscriptionWithReuse(context.Background(), &AssignSubscriptionInput{
		UserID: 41, GroupID: 43, ValidityDays: 5, Notes: "duplicate assignment",
	})
