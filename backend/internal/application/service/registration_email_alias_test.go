...
func TestNormalizeEmailForAliasDedup(t *testing.T) {
	cases := []struct {
		...
		{"custom domain plus stays significant", "user+tag@example.com", "user+tag@example.com"},
...
func TestNormalizeEmailForAliasDedupKeepsDistinctInboxes(t *testing.T) {
	...
	require.NotEqual(t,
		NormalizeEmailForAliasDedup("sales+east@custom.example"),
		NormalizeEmailForAliasDedup("sales+west@custom.example"),
	)
}

func TestEmailAliasDedupProbes(t *testing.T) {
	require.ElementsMatch(t,
		[]EmailAliasProbe{{Local: "someone", Domain: "gmailcom", MatchPlusSuffix: true}, {Local: "someone", Domain: "googlemailcom", MatchPlusSuffix: true}},
		EmailAliasDedupProbes("Some.One+tag@gmail.com"),
	)
	require.ElementsMatch(t,
		[]EmailAliasProbe{{Local: "daxis2026", Domain: "gmailcom", MatchPlusSuffix: true}, {Local: "daxis2026", Domain: "googlemailcom", MatchPlusSuffix: true}},
		EmailAliasDedupProbes("d.axis.2026@googlemail.com."),
	)
	require.Equal(t,
		[]EmailAliasProbe{{Local: "firstlast+tag", Domain: "qqcom"}},
