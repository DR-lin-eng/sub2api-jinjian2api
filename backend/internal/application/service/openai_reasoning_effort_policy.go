...
// WithOpenAIReasoningEffortPolicy binds a group policy after the concrete
// request target has been resolved to OpenAI. Copying mappings prevents retries
// and asynchronous forwarding from observing later slice mutations.
...
// ApplyOpenAIReasoningEffortPolicyFromContext applies a bound policy. An
// unbound request is returned byte-for-byte unchanged.
...
func ApplyOpenAIReasoningEffortPolicy(body []byte, maxEffort string, mappings []ReasoningEffortMapping) ([]byte, bool) {
	if len(body) == 0 || (maxEffort == "" && len(mappings) == 0) {
		return body, false
	}
	maxRank, hasMax := reasoningEffortRank(maxEffort)
	if !hasMax && len(mappings) == 0 {
		return body, false
	}
