...
const (
	...
	// accounts, it admits API-key accounts that explicitly force those requests
	// through their configured /v1/images/generations endpoint.
	OpenAIEndpointCapabilityResponsesOrForcedImageAPI OpenAIEndpointCapability = "responses_or_forced_image_api"
)
...
func (a *Account) SupportsOpenAIEndpointCapability(capability OpenAIEndpointCapability) bool {
	...
	switch capability {
	case OpenAIEndpointCapabilityChatCompletions:
	case OpenAIEndpointCapabilityResponsesOrForcedImageAPI:
...
func (a *Account) GrokMediaGenerationEligibility() (bool, string) {
	...
	billing, observed := grokBillingEligibilitySnapshotFromExtra(a.Extra)
	if !observed {
		return false, "billing_unobserved"
	}
	if billing.status == 403 || billing.weeklyStatus == 403 || billing.monthlyStatus == 403 {
		return false, "billing_forbidden"
	}
	if isKnownGrokFreeAccount(a) {
		return false, "billing_free_tier"
	}
	if !billing.usageObserved && !billing.usedObserved && !(billing.monthlyLimitObserved && billing.monthlyLimitCents > 0) && strings.TrimSpace(billing.plan) == "" {
...
func (a *Account) SupportsOpenAIImageCapability(capability OpenAIImagesCapability) bool {
	...
	case OpenAIImagesCapabilityForcedAPI:
		return a.ForceOpenAIImageAPI()
	default:
		return true
	}
}
