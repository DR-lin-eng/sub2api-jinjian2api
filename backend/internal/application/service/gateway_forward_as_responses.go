...
func adaptResponsesClientToolsForAnthropic(body []byte) ([]byte, apicompat.ResponsesClientToolMapping, error) {
	if !responsesClientToolsNeedAdaptation(body) {
...
func responsesClientToolsNeedAdaptation(body []byte) bool {
	additionalToolsMarker := bytes.Contains(body, []byte(`"additional_tools"`))
	customMarker := bytes.Contains(body, []byte(`"custom"`))
	toolSearchMarker := bytes.Contains(body, []byte(`"tool_search"`))
	namespaceMarker := bytes.Contains(body, []byte(`"namespace"`))
	if !additionalToolsMarker && !customMarker && !toolSearchMarker && !namespaceMarker && !bytes.Contains(body, []byte(`\u`)) {
		return false
	}

	if additionalToolsMarker && gjsonArrayContainsResponsesToolType(gjson.GetBytes(body, "input"), "additional_tools") {
		return true
	}
	if !customMarker && !toolSearchMarker && !namespaceMarker && !bytes.Contains(body, []byte(`\u`)) {
		return false
	}
	return gjsonArrayContainsResponsesToolType(gjson.GetBytes(body, "tools"), "custom", "tool_search", "namespace")
}

func gjsonArrayContainsResponsesToolType(items gjson.Result, types ...string) bool {
	if !items.IsArray() {
		return false
	}
	found := false
...
func (s *GatewayService) handleResponsesStreamingResponse(
	...
	writeEvent := func(evt apicompat.ResponsesStreamEvent) bool {
		if clientToolRestorer == nil {
			...
			if _, err := c.Writer.Write(reverseToolNamesIfPresent(c, []byte(sse))); err != nil {
