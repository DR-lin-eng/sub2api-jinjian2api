...
func (s *OpenAIGatewayService) forwardGrokResponses(
	...
	mixedCacheIntent := inspectGrokFreeToolCacheIntent(patchedBody)
	patchedBody, err = applyGrokResponsesCacheIdentityWithIntent(patchedBody, hasGrokResponsesToolIntent(body), cacheIdentity, account.IsGrokOAuth())
	if err != nil {
	...
	patchedBody, err = applyGrokFreeRequestToolCacheRouteWithIntent(c, patchedBody, mixedCacheIntent, account, cacheIdentity)
	if err != nil {
...
func (s *OpenAIGatewayService) updateGrokUsageSnapshot(ctx context.Context, account *Account, snapshot *xai.QuotaSnapshot) {
	...
	// Pool-mode accounts retain the snapshot for observability while the
	// upstream pool remains responsible for account health and rotation.
...
func (s *OpenAIGatewayService) handleGrokAccountUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte) {
	...
	case http.StatusPaymentRequired:
		s.tempUnscheduleGrok(ctx, account, grokPaymentRequiredCooldown, grokPaymentRequiredReason)
