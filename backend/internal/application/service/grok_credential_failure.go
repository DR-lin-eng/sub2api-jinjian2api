...
func (s *OpenAIGatewayService) blockGrokCredentialRuntime(account *Account, until time.Time, reason string) func() {
	...
	_, hadBeforeTempUnsched := s.openaiAccountRuntimeTempUnsched.Load(account.ID)
	...
	return func() {
		...
		if hadBefore {
			...
			if hadBeforeTempUnsched {
				s.openaiAccountRuntimeTempUnsched.Store(account.ID, struct{}{})
			} else {
