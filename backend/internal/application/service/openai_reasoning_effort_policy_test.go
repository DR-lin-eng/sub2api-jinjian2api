...
var (
	reasoningEffortPolicyBenchmarkBody    []byte
	reasoningEffortPolicyBenchmarkChanged bool
)

func BenchmarkApplyOpenAIReasoningEffortPolicy(b *testing.B) {
	body := []byte(`{"model":"gpt-5.6","input":"hello"}`)
	b.Run("disabled", func(b *testing.B) {
		b.ReportAllocs()
		for range b.N {
			reasoningEffortPolicyBenchmarkBody, reasoningEffortPolicyBenchmarkChanged = ApplyOpenAIReasoningEffortPolicy(body, "", nil)
		}
	})
	b.Run("enabled_without_request_field", func(b *testing.B) {
		b.ReportAllocs()
		for range b.N {
			reasoningEffortPolicyBenchmarkBody, reasoningEffortPolicyBenchmarkChanged = ApplyOpenAIReasoningEffortPolicy(body, "medium", nil)
		}
	})
	b.Run("enabled_with_cap", func(b *testing.B) {
		bodyWithEffort := []byte(`{"model":"gpt-5.6","reasoning":{"effort":"xhigh"}}`)
		b.ReportAllocs()
		for range b.N {
			reasoningEffortPolicyBenchmarkBody, reasoningEffortPolicyBenchmarkChanged = ApplyOpenAIReasoningEffortPolicy(bodyWithEffort, "medium", nil)
		}
	})
}
