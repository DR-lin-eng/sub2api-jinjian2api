...
func applyGrokResponsesCacheIdentity(body, intentSourceBody []byte, identity string, injectFreeTierTools bool) ([]byte, error) {
	return applyGrokResponsesCacheIdentityWithIntent(body, hasGrokResponsesToolIntent(intentSourceBody), identity, injectFreeTierTools)
}

func applyGrokResponsesCacheIdentityWithIntent(body []byte, hasToolIntent bool, identity string, injectFreeTierTools bool) ([]byte, error) {
...
func applyGrokFreeMessagesFunctionToolCacheRoute(body, intentSourceBody []byte, account *Account, cacheIdentity string) ([]byte, error) {
	...
	return applyGrokFreeToolCacheRouteWithIntent(body, inspectGrokFreeToolCacheIntent(intentSourceBody), account, cacheIdentity, allowPureClientTools, true)
}
...
func applyGrokFreeRequestToolCacheRoute(c *gin.Context, body, intentSourceBody []byte, account *Account, cacheIdentity string) ([]byte, error) {
	return applyGrokFreeRequestToolCacheRouteWithIntent(c, body, inspectGrokFreeToolCacheIntent(intentSourceBody), account, cacheIdentity)
}

func applyGrokFreeRequestToolCacheRouteWithIntent(c *gin.Context, body []byte, intent grokFreeToolCacheIntent, account *Account, cacheIdentity string) ([]byte, error) {
	...
	return applyGrokFreeToolCacheRouteWithIntent(body, intent, account, cacheIdentity, allowPureClientTools, allowPureClientTools)
}
...
type grokFreeToolCacheIntent struct {
	functionTools bool
	toolsDisabled bool
}

func inspectGrokFreeToolCacheIntent(body []byte) grokFreeToolCacheIntent {
	tools := gjson.GetBytes(body, "tools")
	toolChoice := gjson.GetBytes(body, "tool_choice")
	return grokFreeToolCacheIntent{
		functionTools: isGrokFreeCacheFunctionToolIntent(tools, toolChoice),
		toolsDisabled: toolChoice.Type == gjson.String && strings.TrimSpace(toolChoice.String()) == grokFreeCacheDisabledToolChoice,
	}
}

func applyGrokFreeToolCacheRouteWithIntent(body []byte, intent grokFreeToolCacheIntent, account *Account, cacheIdentity string, allowPureClientTools, allowFunctionSearch bool) ([]byte, error) {
	if strings.TrimSpace(cacheIdentity) == "" || !isKnownGrokFreeAccount(account) {
		return body, nil
	}
	if !intent.functionTools {
		return body, nil
	}
	if intent.toolsDisabled {
