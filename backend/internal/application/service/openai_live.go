...
func (s *OpenAIGatewayService) liveConcurrencyCache() (LiveConcurrencyCache, error) {
	...
	_, ok := s.concurrencyService.cache.(LiveConcurrencyCache)
	if !ok {
		return nil, ErrLiveUnavailable
	}
	return s.concurrencyService, nil
}

func (s *ConcurrencyService) rawLiveConcurrencyCache() (LiveConcurrencyCache, error) {
	if s == nil || s.cache == nil {
		return nil, ErrLiveUnavailable
	}
...
func (s *ConcurrencyService) AcquireLiveLease(
	ctx context.Context,
	accountID int64,
	accountMax int,
	userID int64,
	...
	cache, err := s.rawLiveConcurrencyCache()
	if err != nil {
		return false, err
	}
	...
	localLease := &localLiveLease{}
	userAllowance := boolIntService(replacements.User)
	userSlot, ok := s.acquireStandaloneSlotWithAllowance(&s.localUserSlots, userID, userMax, userAllowance)
	if !ok {
		return false, errors.New("standalone Live user slot registry is full")
	}
	if userSlot == nil || !userSlot.Acquired {
		return false, nil
	}
	localLease.userRelease = userSlot.ReleaseFunc

	apiKeyAllowance := boolIntService(replacements.APIKey)
	apiKeySlot, ok := s.acquireStandaloneSlotWithAllowance(&s.localAPIKeySlots, apiKeyID, apiKeyMax, apiKeyAllowance)
	if !ok {
		localLease.release()
		return false, errors.New("standalone Live API-key slot registry is full")
	}
	if apiKeySlot == nil || !apiKeySlot.Acquired {
		localLease.release()
		return false, nil
	}
	localLease.apiKeyRelease = apiKeySlot.ReleaseFunc

	// Standalone user/API-key request slots live in-process, so only the account
	// replacement exists in Redis. Redis still stores all three Live members for
	// lifecycle refresh and a later deployment-mode transition.
	redisReplacements := replacements
	redisReplacements.User = false
	redisReplacements.APIKey = false
	acquired, acquireErr := cache.AcquireLiveLease(
		ctx,
		accountID,
		accountMax,
		userID,
	...
	if existing, loaded := s.localLiveLeases.LoadOrStore(leaseID, localLease); loaded {
		localLease.release()
		if existing == nil {
			return false, errors.New("standalone Live lease registry is inconsistent")
		}
	}
	return true, nil
}

func (s *ConcurrencyService) RefreshLiveLease(
	ctx context.Context,
	...
	if s.standaloneRequestSlots.Load() {
		if _, ok := s.localLiveLeases.Load(leaseID); !ok {
			return false, nil
		}
	}
	return cache.RefreshLiveLease(ctx, accountID, userID, apiKeyID, leaseID)
}

func (s *ConcurrencyService) ReleaseLiveLease(
	ctx context.Context,
	accountID, userID, apiKeyID int64,
	leaseID string,
) error {
	cache, cacheErr := s.rawLiveConcurrencyCache()
	var releaseErr error
	if cacheErr == nil {
		releaseErr = cache.ReleaseLiveLease(ctx, accountID, userID, apiKeyID, leaseID)
	}
	if local, ok := s.localLiveLeases.LoadAndDelete(leaseID); ok {
		if lease, valid := local.(*localLiveLease); valid {
			lease.release()
		}
	}
	if cacheErr != nil {
...
func boolIntService(value bool) int {
	if value {
		return 1
	}
	return 0
...
func (s *OpenAIGatewayService) CreateLiveCall(
	...
	for attempt := 0; attempt <= 3; attempt++ {
		...
		acquired, acquireErr := liveCache.AcquireLiveLease(
			...
			leaseID,
			LiveConcurrencyReplacements{Account: true, User: true, APIKey: true},
		)
		if acquireErr != nil || !acquired {
			selection.ReleaseFunc()
			if acquireErr != nil {
...
func (s *OpenAIGatewayService) observeLiveCall(record *LiveCallRecord) {
	...
	if claimErr != nil {
		// store 报错时无法确认控制权归属；按 ExpiresAt 兜底 finalize，避免 usage log 与租约释放丢失。
...
func (s *OpenAIGatewayService) waitForLiveObserverRetry(record *LiveCallRecord) bool {
	...
	if getErr != nil && !errors.Is(getErr, ErrLiveCallNotFound) {
		// store 报错不等于控制权被接管，交回 observeLiveCall 做有限重试与到期兜底。
		return true
	}
