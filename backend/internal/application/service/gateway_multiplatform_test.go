...
func (m *mockAccountRepoForPlatform) ListModelAvailabilityCandidates(_ context.Context, groupID *int64, platforms []string, includeGrouped bool) ([]Account, error) {
	...
	for _, acc := range m.accounts {
		...
		if groupID != nil {
			...
			if !inGroup {
				for _, accountGroupID := range acc.GroupIDs {
...
func (m *mockGroupRepoForGateway) ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, status, search string) ([]Group, *pagination.PaginationResult, error) {
	return nil, nil, nil
}
