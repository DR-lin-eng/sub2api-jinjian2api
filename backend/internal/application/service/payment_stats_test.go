...
func TestComputeBasicStatsGroupsAmountsByCurrency(t *testing.T) {
	...
	require.Equal(t, 25.0, stats.TotalAmount)
	require.Equal(t, 20.0, stats.TodayAmount)
	require.Equal(t, 8.33, stats.AvgAmount)
	require.Equal(t, CurrencyAmounts{"CNY": 15, "USD": 10}, stats.TotalAmountByCurrency)
	require.Equal(t, CurrencyAmounts{"CNY": 10, "USD": 10}, stats.TodayAmountByCurrency)
	require.Equal(t, CurrencyAmounts{"CNY": 7.5, "USD": 10}, stats.AvgAmountByCurrency)
...
func TestPaymentDashboardBreakdownsGroupAmountsAndRankingsByCurrency(t *testing.T) {
	...
	require.Equal(t, []DailyStats{
		{Date: "2026-07-24", Amount: 15.56, AmountByCurrency: CurrencyAmounts{"CNY": 15.56}, Count: 2},
		{Date: "2026-07-25", Amount: 30, AmountByCurrency: CurrencyAmounts{"USD": 30}, Count: 2},
	}, daily)

	methods := buildMethodDistribution(orders)
	require.Equal(t, []PaymentMethodStat{
		{Type: "alipay", Amount: 10, AmountByCurrency: CurrencyAmounts{"USD": 10}, Count: 1},
		{Type: "stripe", Amount: 35.56, AmountByCurrency: CurrencyAmounts{"CNY": 15.56, "USD": 20}, Count: 3},
	}, methods)

	users := buildTopUsers(orders)
	require.Equal(t, []TopUserStat{
		{UserID: 1, Email: "alice@example.com", Amount: 25.56},
		{UserID: 2, Email: "bob@example.com", Amount: 20},
	}, users)

	usersByCurrency := buildTopUsersByCurrency(orders)
