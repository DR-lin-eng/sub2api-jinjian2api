...
//
// Normalization rules:
//   - All domains: lowercase, trim, and drop the FQDN root dot.
//   - Gmail family (gmail.com / googlemail.com): additionally strip the
//     local-part "+suffix", remove dots from the local part, and fold the
//     domain to gmail.com. Plus signs remain significant for every other
//     domain because custom mail systems may treat them as ordinary characters.
//
...
// dot-stripped local part, Domain the dot-stripped candidate domain, and
// MatchPlusSuffix whether stored Gmail suffix variants also need a prefix probe.
// Dots are removed on both sides of that comparison so one probe per domain also
// covers the Gmail dot trick and the FQDN root dot. The over-matching this
// introduces (dots stay significant outside the Gmail family) is filtered by
// re-checking every candidate with NormalizeEmailForAliasDedup.
type EmailAliasProbe struct {
	Local           string
	Domain          string
	MatchPlusSuffix bool
}
...
func EmailAliasDedupProbes(email string) []EmailAliasProbe {
	...
	matchPlusSuffix := isGmailFamilyDomain(domain)
	if matchPlusSuffix {
		local = stripEmailPlusSuffix(local)
	}
	probeLocal := strings.ReplaceAll(local, ".", "")
	if probeLocal == "" {
		return nil
	}
	...
	for _, candidate := range domains {
		probes = append(probes, EmailAliasProbe{
			...
			MatchPlusSuffix: matchPlusSuffix,
		})
	}
	return probes
}
