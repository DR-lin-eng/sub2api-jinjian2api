...
func (s *SubscriptionService) updateExistingSubscriptionTerm(
	...
	return s.withSubscriptionUpdateTx(ctx, func(txCtx context.Context) error {
		...
		if assignmentSemantics {
			...
			// AssignSubscription is idempotent. A stale pre-lock read must not
			// reactivate a suspended row or extend a row another request renewed.
			if existingSub.Status == SubscriptionStatusSuspended || !isExpired {
				return nil
			}
		}
