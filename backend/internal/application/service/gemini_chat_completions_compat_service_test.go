...
func TestGeminiInlineBase64ValidationDoesNotCopyLargeImages(t *testing.T) {
	data := strings.Repeat("QUJD", 1<<18)
	require.True(t, isValidBase64(data))
	...
	for _, invalid := range []string{"", "A", "AA=A", "AA==AAAA", "not-valid!!!="} {
		require.Falsef(t, isValidBase64(invalid), "accepted invalid base64 %q", invalid)
	}
}
