...
type ollamaUsageSettingsCacheRepo struct {
	*upstreamBillingProbeSettingRepo
	reads     atomic.Int64
	failReads atomic.Bool
}

func (r *ollamaUsageSettingsCacheRepo) GetValue(ctx context.Context, key string) (string, error) {
	r.reads.Add(1)
	if r.failReads.Load() {
		return "", errors.New("settings read failed")
	}
	return r.upstreamBillingProbeSettingRepo.GetValue(ctx, key)
}
...
func TestOllamaCloudUsageRunnerSettingsCacheBoundsDisabledReadsAndReturnsCopies(t *testing.T) {
	repo := &ollamaUsageSettingsCacheRepo{upstreamBillingProbeSettingRepo: &upstreamBillingProbeSettingRepo{}}
	svc := NewOllamaCloudUsageService(nil, nil, NewSettingService(repo, nil), nil, false)
	t.Cleanup(svc.Stop)
	now := time.Date(2026, time.July, 23, 12, 0, 0, 0, time.UTC)
	svc.now = func() time.Time { return now }

	settings, err := svc.getRunnerSettings(context.Background())
	require.NoError(t, err)
	require.False(t, settings.Enabled)
	require.Equal(t, int64(1), repo.reads.Load())
	settings.Enabled = true
	settings.IntervalMinutes = 999

	cached, err := svc.getRunnerSettings(context.Background())
	require.NoError(t, err)
	require.False(t, cached.Enabled, "callers must receive a cache copy")
	require.Equal(t, ollamaCloudUsageDefaultIntervalMinutes, cached.IntervalMinutes)
	now = now.Add(ollamaCloudUsageDisabledSettingsTTL - time.Second)
	_, err = svc.getRunnerSettings(context.Background())
	require.NoError(t, err)
	require.Equal(t, int64(1), repo.reads.Load(), "disabled runner must not query settings every minute")
	...
	require.NoError(t, err)
	require.Equal(t, int64(3), repo.reads.Load(), "public settings reads must bypass the runner cache")
}

func TestOllamaCloudUsageUpdateSettingsRefreshesEnabledRunnerCacheImmediately(t *testing.T) {
	repo := &ollamaUsageSettingsCacheRepo{upstreamBillingProbeSettingRepo: &upstreamBillingProbeSettingRepo{}}
	svc := NewOllamaCloudUsageService(nil, nil, NewSettingService(repo, nil), nil, false)
	t.Cleanup(svc.Stop)
	now := time.Date(2026, time.July, 23, 12, 0, 0, 0, time.UTC)
	...
	require.NoError(t, err)
	require.Equal(t, int64(1), repo.reads.Load())
	require.NoError(t, svc.UpdateSettings(context.Background(), &OllamaCloudUsageSettings{
		Enabled: true, IntervalMinutes: 90,
	}))
	...
	require.True(t, settings.Enabled)
	require.Equal(t, 90, settings.IntervalMinutes)
	require.Equal(t, int64(1), repo.reads.Load(), "local update must replace the cache without another read")
	now = now.Add(ollamaCloudUsageEnabledSettingsTTL - time.Second)
	_, err = svc.getRunnerSettings(context.Background())
	require.NoError(t, err)
	require.Equal(t, int64(1), repo.reads.Load())
	...
	require.NoError(t, err)
	require.True(t, settings.Enabled)
	require.Equal(t, int64(2), repo.reads.Load(), "enabled instances retain one-minute cross-instance convergence")
}

func TestOllamaCloudUsageRunnerSettingsReadErrorDoesNotExtendExpiredCache(t *testing.T) {
	repo := &ollamaUsageSettingsCacheRepo{upstreamBillingProbeSettingRepo: &upstreamBillingProbeSettingRepo{}}
	svc := NewOllamaCloudUsageService(nil, nil, NewSettingService(repo, nil), nil, false)
	t.Cleanup(svc.Stop)
	now := time.Date(2026, time.July, 23, 12, 0, 0, 0, time.UTC)
	...
	_, err := svc.getRunnerSettings(context.Background())
	require.NoError(t, err)
	now = now.Add(ollamaCloudUsageDisabledSettingsTTL)
	repo.failReads.Store(true)
	_, err = svc.getRunnerSettings(context.Background())
	require.ErrorContains(t, err, "settings read failed")
	require.Equal(t, int64(2), repo.reads.Load())

	repo.failReads.Store(false)
	_, err = svc.getRunnerSettings(context.Background())
	require.NoError(t, err)
	require.Equal(t, int64(3), repo.reads.Load(), "a failed refresh must not renew the expired cache")
}
...
func TestOllamaLabelIndexUsesBoundedLinearTextCollection(t *testing.T) {
	var page strings.Builder
	_, err := page.WriteString(`<html><body><main>`)
	require.NoError(t, err)
	for index := 0; index < 2_000; index++ {
		_, err = fmt.Fprintf(&page, `<div><span>unrelated setting %d</span><span>%s</span></div>`, index, strings.Repeat("x", 32))
		require.NoError(t, err)
	}
	_, err = page.WriteString(`<section><div><span><strong>5 hour usage</strong></span><span>42% used</span></div></section>`)
	require.NoError(t, err)
	_, err = page.WriteString(`</main></body></html>`)
	require.NoError(t, err)
	doc, err := html.Parse(strings.NewReader(page.String()))
	require.NoError(t, err)

	labels := buildOllamaLabelIndex(doc)
	label := labels.find(ollamaFiveHourUsageAliases)

	require.NotNil(t, label)
	require.Equal(t, "span", label.Data)
	require.Equal(t, "5 hour usage", normalizedNodeText(label))
	require.Greater(t, len(labels.candidates), 4_000)
	for _, candidate := range labels.candidates {
		if !candidate.overflow {
			require.LessOrEqual(t, len(candidate.text), ollamaLabelTextMaxBytes)
		}
	}
}

func BenchmarkFindLabelElementBoundedLinear(b *testing.B) {
	var page strings.Builder
	if _, err := page.WriteString(`<html><body><main>`); err != nil {
		b.Fatal(err)
	}
	for index := 0; index < 4_000; index++ {
		if _, err := fmt.Fprintf(&page, `<div><span>setting %d</span><span>value %d</span></div>`, index, index); err != nil {
			b.Fatal(err)
		}
	}
	if _, err := page.WriteString(`<div><span>7 day usage</span><span>14.2% used</span></div></main></body></html>`); err != nil {
		b.Fatal(err)
	}
	doc, err := html.Parse(strings.NewReader(page.String()))
	if err != nil {
	...
	b.ResetTimer()
	for b.Loop() {
		if label := findLabelElement(doc, ollamaSevenDayUsageAliases); label == nil {
			b.Fatal("usage label not found")
		}
	}
}

func TestDecodeOllamaCloudUsageSnapshotKeepsLegacyScheduleCompatible(t *testing.T) {
	nextRefresh := time.Date(2026, time.July, 23, 16, 0, 0, 123456789, time.UTC)
	snapshot := decodeOllamaCloudUsageSnapshot(map[string]any{
		OllamaCloudUsageSnapshotExtraKey: map[string]any{
			"status":          OllamaCloudUsageStatusOK,
			"last_attempt_at": nextRefresh.Add(-time.Hour).Format(time.RFC3339Nano),
			"next_refresh_at": nextRefresh.Format(time.RFC3339Nano),
		},
	})

	require.NotNil(t, snapshot)
	require.Equal(t, nextRefresh, snapshot.NextRefreshAt)
	require.Zero(t, snapshot.NextRefreshUnix)
}
...
func TestOllamaCloudUsageRefreshSingleflightAndRunnerDeduplicateSharedGroup(t *testing.T) {
	...
	for range 2 {
		...
		// The second goroutine may reach singleflight just after the first
		// completes. In that valid ordering it observes the manual rate limit;
		// either ordering must still issue only one upstream request.
...
func TestOllamaCloudUsageRefreshUsesFixedURLCookieAndNoRedirects(t *testing.T) {
	...
	require.Equal(t, state.Snapshot.NextRefreshAt.Unix(), state.Snapshot.NextRefreshUnix)
