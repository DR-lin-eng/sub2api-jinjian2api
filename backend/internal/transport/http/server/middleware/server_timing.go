...
// ServerTiming collects timing for local administrator panel requests when enabled.
...
// ServerTimingHeaderValue returns a timing value only to the local administrator.
...
func shouldCollectServerTiming(c *gin.Context) bool {
	...
	return isAdminUIRequest(c) || isPanelTimingPath(c.Request.URL.Path)
}
...
// isPanelTimingPath reports whether the path belongs to the local panel surface.
func isPanelTimingPath(path string) bool {
