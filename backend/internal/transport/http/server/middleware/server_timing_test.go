...
func TestServerTimingScopesAndRoleGate(t *testing.T) {
	tests := []struct {
		...
		{name: "unauthenticated panel path", enabled: true, path: "/api/v1/keys"},
		...
		{name: "auth me path", enabled: true, path: "/api/v1/auth/me", role: "admin", wantHeader: true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			recorder := runServerTimingRequest(t, tt.enabled, tt.path, tt.adminMarker, tt.role, func(c *gin.Context) {
...
func TestIsPanelTimingPath(t *testing.T) {
	tests := []struct {
		path string
		want bool
	}{
		...
		{"/api/v1/auth/oauth/bind-token", false},
		...
		{"/api/v1/groups/rates", false},
		{"/api/v1/groups", false},
		{"/api/v1/channels/available", false},
	...
	for _, tt := range tests {
		t.Run(tt.path, func(t *testing.T) {
			if got := isPanelTimingPath(tt.path); got != tt.want {
				t.Fatalf("isPanelTimingPath(%q) = %v, want %v", tt.path, got, tt.want)
			}
		})
	}
}
