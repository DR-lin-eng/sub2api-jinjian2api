...
type openAIPassthroughFailoverState struct {
	...
	sanitizeComputed bool
	sanitizedBody    []byte
	sanitizedChanged bool
	sanitizeErr      error
	sanitizeLogged   bool
}
...
func (h *OpenAIGatewayHandler) deriveOpenAIForwardAttemptBody(
	...
	if !state.sanitizeComputed {
		state.sanitizedBody, state.sanitizedChanged, state.sanitizeErr = service.SanitizeOpenAICrossModeFailoverReasoning(canonicalBody)
		state.sanitizeComputed = true
	}
	if state.sanitizeErr != nil {
		if reqLog != nil && !state.sanitizeLogged {
			reqLog.Warn("openai.failover_cross_mode_reasoning_sanitize_failed",
				zap.Int64("account_id", account.ID),
				zap.Error(state.sanitizeErr),
			)
			state.sanitizeLogged = true
		}
		return canonicalBody
	}
	if !state.sanitizedChanged {
		return canonicalBody
	}
	if reqLog != nil && !state.sanitizeLogged {
