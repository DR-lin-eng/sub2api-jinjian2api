...
//  3. 平台过滤：普通分组只保留自身平台模型；Composite 分组按渠道已配置的具体模型平台展开；
...
// Composite 分组会展开到每个已配置模型的平台；无模型时保留 composite 空状态。
...
func buildPlatformSections(
	...
	if len(compositeGroups) > 0 {
		...
		if len(modelPlatforms) == 0 {
			groupsByPlatform[service.PlatformComposite] = append(groupsByPlatform[service.PlatformComposite], compositeGroups...)
		} else {
