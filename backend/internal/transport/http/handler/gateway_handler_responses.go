...
func (h *GatewayHandler) Responses(c *gin.Context) {
	...
	if openai_compat.ShouldRejectGPT56ResponsesExplicitCache(reqModel, body) {
		h.responsesInvalidRequestParamErrorResponse(
			...
			"Unsupported parameter: 'explicit_cache' is not supported for GPT-5.6 Responses requests. Use prompt_cache_options with prompt_cache_breakpoint for explicit prompt caching.",
		...
		if !selection.Acquired {
			...
			if err := h.gatewayService.EnsureAccountSchedulableAfterWait(c.Request.Context(), apiKey.GroupID, sessionHash, account.ID); err != nil {
				...
				if errors.Is(err, service.ErrAccountSchedulingChanged) {
					reqLog.Info("gateway.responses.account_reschedule_after_wait", zap.Int64("account_id", account.ID))
					fs.ExcludeAccount(account.ID)
					continue
				}
				reqLog.Warn("gateway.responses.account_recheck_after_wait_failed", zap.Int64("account_id", account.ID), zap.Error(err))
				h.responsesErrorResponse(c, http.StatusServiceUnavailable, "api_error", "Account scheduling state is unavailable")
				return
			}
		}
