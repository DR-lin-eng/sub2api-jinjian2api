...
func (h *AuthHandler) Register(c *gin.Context) {
	...
	if err := h.authService.VerifyHumanVerificationForRegister(c.Request.Context(), humanVerificationProof(req.CaptchaToken, req.TurnstileToken, req.TencentCaptchaTicket, req.TencentCaptchaRandstr), ip.GetClientIP(c), req.VerifyCode); err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *AuthHandler) SendVerifyCode(c *gin.Context) {
	...
	if err := h.authService.VerifyHumanVerification(c.Request.Context(), humanVerificationProof(req.CaptchaToken, req.TurnstileToken, req.TencentCaptchaTicket, req.TencentCaptchaRandstr), ip.GetClientIP(c)); err != nil {
		response.ErrorFrom(c, err)
		return
	}
