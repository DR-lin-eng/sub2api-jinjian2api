...
func (s *passkeyCaptchaVerifierStub) VerifyTicket(
	_ context.Context,
...
func (r *passkeySwitchSettingRepo) GetMultiple(_ context.Context, keys []string) (map[string]string, error) {
	if r.err != nil {
		return nil, r.err
	}
...
func newTencentProtectedPasskeyHandler(t *testing.T) (*PasskeyHandler, *passkeyCaptchaVerifierStub, *passkeyBeginSessionStoreStub) {
	...
	authHandler := &AuthHandler{authService: authService, settingSvc: settings, cfg: cfg}
	sessions := &passkeyBeginSessionStoreStub{}
	passkeys, err := service.NewPasskeyService(cfg, nil, sessions, nil)
	require.NoError(t, err)
	return NewPasskeyHandler(passkeys, authHandler, settings), verifier, sessions
}
