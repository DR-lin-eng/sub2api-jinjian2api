...
func TestDeriveOpenAIForwardAttemptBody_SanitizationSticksAcrossBedrockRetries(t *testing.T) {
	...
	require.True(t, state.sanitizeComputed)
	require.Same(t, &second[0], &retry[0], "same-account retry must reuse the cached sanitized body")
	require.Same(t, &second[0], &nextAccount[0], "later accounts must reuse the cached sanitized body")
	require.JSONEq(t, kiroReasoningCanonicalBody, string(canonical), "canonical forwardBody must never be mutated")
}
