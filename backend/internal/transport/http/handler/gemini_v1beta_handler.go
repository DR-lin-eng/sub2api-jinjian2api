...
import (
	...
	"github.com/Wei-Shaw/sub2api/internal/shared/gemini"
...
func (h *GatewayHandler) GeminiV1BetaModels(c *gin.Context) {
	...
	if decision := h.checkSecurityAudit(c, reqLog, apiKey, authSubject, service.PromptAuditProtocolGemini, modelName, body); decision != nil && !decision.AllowNextStage {
	...
	// 2) select account (sticky session based on request body)
