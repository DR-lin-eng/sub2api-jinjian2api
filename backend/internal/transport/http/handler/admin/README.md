...
文件按资源前缀组织：`account_*`, `group_*`, `setting_*`, `ops_*`, `channel_*`, `proxy_*`, `backup_*`, `audit_*`, `cluster_*`, `content_moderation_*`。`*_data.go` 只放请求/响应结构，`*_handler.go` 放端点实现，`*_test.go` 与对应前缀并排。
...
| `setting_handler_update_persistence.go` | 持久化系统设置与独立网关策略 |
