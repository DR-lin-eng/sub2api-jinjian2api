...
// GetDashboardImageGenerationStats returns image generation latency, resolution,
// throughput, and concurrency statistics.
// GET /api/v1/admin/ops/dashboard/image-generation-stats
func (h *OpsHandler) GetDashboardImageGenerationStats(c *gin.Context) {
	if h.opsService == nil {
		response.Error(c, http.StatusServiceUnavailable, "Ops service not available")
		return
	}
	...
	data, err := h.opsService.GetImageGenerationStats(c.Request.Context(), filter)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
