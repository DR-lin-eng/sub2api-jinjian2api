...
func (r *userRepository) Update(ctx context.Context, userIn *service.User, fields service.UserUpdateFields) error {
	...
	if fields.SchedulingTier {
		updateOp = updateOp.SetRequestSchedulingTier(int16(service.NormalizeRequestSchedulingTier(userIn.SchedulingTier)))
	}
	if fields.RPMLimit {
...
func existsByEmailAliasWithClient(ctx context.Context, client *dbent.Client, email string) (bool, error) {
	...
	for _, probe := range probes {
		preds = append(preds, dotStrippedEmailEQ(probe.Local+"@"+probe.Domain))
		if probe.MatchPlusSuffix {
			// Gmail "+后缀"的内容未知，只能按前缀匹配。
			preds = append(preds, dotStrippedEmailLike(escapeLikeWildcards(probe.Local)+"+%@"+escapeLikeWildcards(probe.Domain)))
		}
	}
