...
func TestListDueOllamaCloudUsageAccountsSeparatesManagedUnixFromMalformedValues(t *testing.T) {
	ctx := context.Background()
	tx := testEntTx(t)
	repo := newAccountRepositoryWithSQL(tx.Client(), tx, nil)
	now := time.Date(2026, time.July, 22, 14, 0, 0, 0, time.UTC)
	...
	create := func(name, nextRefreshAt string, nextRefreshUnix any) *service.Account {
		t.Helper()
		return mustCreateAccount(t, tx.Client(), &service.Account{
			Name: name, Platform: service.PlatformOpenAI, Type: service.AccountTypeAPIKey,
			...
			Extra: map[string]any{
				...
				service.OllamaCloudUsageSnapshotExtraKey: map[string]any{
					...
					"next_refresh_unix": nextRefreshUnix,
				},
			},
			LastUsedAt: &lastUsed,
		})
	...
	malformedDue := create("ollama-unix-malformed-due", "not-a-time", "invalid")
	steadyDue := create("ollama-unix-steady-due", "not-a-time", now.Add(-time.Minute).Unix())
	_ = create("ollama-unix-steady-future", "not-a-time", now.Add(time.Hour).Unix())
	_ = create("ollama-unix-fractional-future", now.Add(time.Hour).Format(time.RFC3339Nano), 1.5)
	_ = create("ollama-unix-negative-future", now.Add(time.Hour).Format(time.RFC3339Nano), -1)

	accounts, err := repo.ListDueOllamaCloudUsageAccounts(ctx, now, time.Minute, time.Hour, 10)

	require.NoError(t, err, "malformed managed timestamps must stay on the fail-open legacy branch")
	require.Equal(t, []int64{malformedDue.ID, steadyDue.ID}, accountIDs(accounts))
}
...
func TestOllamaCloudUsageGroupWritesAreAtomicAcrossPlatformsAndURLVariants(t *testing.T) {
	...
	configuredGroup, err := repo.ListOllamaCloudUsageGroupAccounts(ctx, []*service.Account{first, different})
	require.NoError(t, err)
	require.ElementsMatch(t, []int64{first.ID, second.ID}, accountIDs(configuredGroup),
		"group resolution must expand only configured siblings for requested API keys")
