...
func invalidateProxyProbeSnapshots(ctx context.Context, exec sqlExecutor, proxyID int64) ([]int64, error) {
	rows, err := exec.QueryContext(ctx, `
		...
		WHERE proxy_id = $1
			AND type = 'apikey'
			AND (
				(platform IN ('openai', 'anthropic', 'gemini', 'antigravity', 'grok')
	...
	if target == nil {
		rows, err = exec.QueryContext(ctx, `
			UPDATE accounts SET proxy_id=NULL, proxy_fallback_origin_id=$1,
				extra=CASE
					WHEN platform IN ('openai', 'anthropic', 'gemini', 'antigravity', 'grok') AND type='apikey' AND extra ? 'upstream_billing_probe'
