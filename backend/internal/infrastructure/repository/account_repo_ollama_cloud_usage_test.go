...
func TestListOllamaCloudUsageGroupAccountsUsesOneStrictBatchQuery(t *testing.T) {
	...
	t.Cleanup(func() { _ = db.Close() })
	var capturedSQL string
	mock.ExpectQuery("WITH requested_keys AS").
	...
	require.Contains(t, query, "FROM unnest($1::text[]) AS requested(api_key)")
	require.Contains(t, query, "CROSS JOIN LATERAL")
	require.Contains(t, query, "credentials ->> 'api_key' = requested_keys.api_key")
	...
	require.Contains(t, query, "credentials ->> 'api_key' <> ''")
	require.Contains(t, query, "jsonb_typeof(extra -> 'ollama_cloud_usage_session') = 'string'")
