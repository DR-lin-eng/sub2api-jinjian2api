...
func TestAuthCacheInvalidationTriggers_CoverSecurityMutationsOnly(t *testing.T) {
	...
	group := mustCreateGroup(t, integrationEntClient, &service.Group{
		Name: fmt.Sprintf("auth-outbox-group-%d", suffix), RateMultiplier: 1,
	})
	...
	_, err := integrationDB.ExecContext(ctx, "UPDATE api_keys SET last_used_at = NOW() WHERE id = $1", key.ID)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	clear()
	_, err = integrationDB.ExecContext(ctx, "UPDATE users SET request_scheduling_tier = 0 WHERE id = $1", user.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "user scheduling tier change must enqueue all active keys")
	clear()
